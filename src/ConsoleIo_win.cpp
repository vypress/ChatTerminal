...
namespace consoleio
{
	...
	*/
	int console_process_input_char(wchar_t ch_in, wstring& strTyped)
	{
		...
		/*
		//If the conversion is not possible in the current locale, wctomb returns Ц1 and errno is set to EILSEQ.
	...
	*/
	...
	{
		return print_line(format, strarg.c_str(), 0);//добавлен ещЄ один параметр, что бы вызвать функцию int print_line(const wchar_t* format, ...)
	}
